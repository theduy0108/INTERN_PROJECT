...
class Data_creating_final_stream_list():
    def __init__(self, data_path, link_save, file_name):
    ...
    def _preprocessing(self, ofs=1000, nfs=125, hcut=0.28, lcut=25):
        ...
        ## resample the fs:
        ecg_data_resample = decimate(ecg_data, int(ofs / nfs))
        # bp_data_resample = decimate(bp_data, int(ofs / nfs))

        ## bandpass filter and baseline removal
        ecg_data_bp = self._butter_bandpass_filter(ecg_data_resample, hcut, lcut, nfs, order=2)
    ...
    def _Final_data_cutting(self, second, overlap):
        ...
        ecg_data = self._preprocessing(ofs=1000, nfs=125, hcut=0.28, lcut=25)
...
if choosing_mode == 'Test Old Data':
    ...
    if len(uploaded_files) == 2:
        a = Data_creating_final_stream_list(link_upload, link_data_save, data_file_name)
        df = a.main()
